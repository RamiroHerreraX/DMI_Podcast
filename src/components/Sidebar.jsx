...
import React from 'react'
import styled from "styled-components";
import {HomeRounded, CloseRounded, InfoRounded, LightModeRounded, DarkModeRounded} from "@mui/icons-material";
import LogoImage from "../images/logopodcast.jpg"
import {Link} from "react-router-dom";
...
const Sidebar = ({menuOpen, setMenuOpen, setDarkMode, darkMode}) => {
  ...
  return (
    <MenuContainer menuOpen={menuOpen}>
      ...
      {menuItems.map((item)=>(
        <Link to={item.link} style={{textDecoration: "none"}} key={item.name}>
          <Elements>
            {item.icon}
            <NavText>{item.name}</NavText>
          </Elements>
